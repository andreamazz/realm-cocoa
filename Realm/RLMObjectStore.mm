...
void RLMRealmSetSchema(RLMRealm *realm, RLMSchema *targetSchema, bool verify) {
    ...
    for (RLMObjectSchema *objectSchema in realm.schema.objectSchema) {
        ...
        if (verify && objectSchema.table) {
            ObjectStore::verify_and_update_column_mapping(realm.group, objectSchema->_objectSchema);
        }
    }
}
...
NSError *RLMUpdateRealmToSchemaVersion(RLMRealm *realm, NSUInteger newVersion, RLMSchema *targetSchema, NSError *(^migrationBlock)()) {
    ...
    // a write transaction
    if (!ObjectStore::is_migration_required(realm.group, newVersion) && RLMRealmGetTables(realm, targetSchema)) {
    ...
    try {
        std::vector<ObjectSchema *> targetObjectSchema;
        for (RLMObjectSchema *obj in targetSchema.objectSchema) {
            targetObjectSchema.push_back(&(obj->_objectSchema));
        }

        bool changed = ObjectStore::update_realm_with_schema(realm.group, newVersion, targetObjectSchema, [=](){ migrationBlock(); });
