...
realm::TableRef ObjectStore::table_for_object_type(realm::Group *group, StringData object_type) {
    return group->get_or_add_table(table_name_for_class(object_type));
}
...
void ObjectStore::verify_and_update_column_mapping(realm::Group *group, ObjectSchema &target_schema) {
    ...
    for (auto current_prop = table_schema.properties.begin(); current_prop != table_schema.properties.end(); current_prop++) {
        ...
        properties.push_back(*target_prop);
    }
    ...
    if (validation_errors.size()) {
        throw ObjectStoreValidationException(validation_errors);
    }

    // set new properties array with correct column alignment
    target_schema.properties = properties;
}
...
static inline bool create_tables(realm::Group *group, std::vector<ObjectSchema *> target_schema, bool update_existing) {
...
bool ObjectStore::update_realm_with_schema(realm::Group *group,
    ...
    for (size_t i = 0; i < schema.size(); i++) {
        ...
        if (table) {
            verify_and_update_column_mapping(group, *target_schema);
        }
    }
